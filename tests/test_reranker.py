...
def test_rerank_drops_chunks_below_min_score(mock_get_model):
    # CrossEncoder.predict() devuelve logits crudos (pueden ser negativos),
    # no probabilidades -- rerank() los pasa por sigmoide antes de comparar
    # con min_score. sigmoid(2.0) ~= 0.88, sigmoid(-2.0) ~= 0.12.
    a, b = _chunk("a", "texto relevante"), _chunk("b", "texto irrelevante")
    mock_get_model.return_value = _fake_model([2.0, -2.0])
...
def test_rerank_can_drop_everything(mock_get_model):
    ...
    mock_get_model.return_value = _fake_model([-3.0, -4.0])
